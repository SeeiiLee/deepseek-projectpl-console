...
| GOVERNANCE_INDEX.md | 权威入口 | 治理总索引的人类可读入口；机器入口与当前状态见下两行 |
| governance/governance-index.json | 权威机器索引 | 权威优先级、固定合同 hash、实时状态入口、任务合同和 fail-closed 规则 |
| governance/current-state.json | 活跃机器状态 | 当前项目身份、A/B 基线、G0 阶段、阻断、保护路径和下一任务；每次交接必须刷新 |
