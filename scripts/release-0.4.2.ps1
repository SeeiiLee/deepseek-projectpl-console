...
$tag = 'v0.4.2'
$bodyText = [string](Get-Content -Raw -Encoding UTF8 'docs\release-notes\0.4.2.md')
...
if ($existing) {
  ...
  $payload = @{
    tag_name = $tag
    name = "DeepSeek Harness Personal v0.4.2"
    body = $bodyText
    draft = $false
    prerelease = $false
  }
...
$assets = @(
  'artifacts\DeepSeek-Harness-Personal-0.4.2-setup-x64.exe',
  'artifacts\DeepSeek-Harness-Personal-0.4.2-setup-x64.exe.sha256',
  'artifacts\DeepSeek-Harness-Personal-0.4.2-setup-x64.exe.blockmap',
  'artifacts\DeepSeek-Harness-Personal-0.4.2-portable-x64.exe',
  'artifacts\DeepSeek-Harness-Personal-0.4.2-portable-x64.exe.sha256'
)
